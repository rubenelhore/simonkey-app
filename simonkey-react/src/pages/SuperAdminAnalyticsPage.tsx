...
const SuperAdminAnalyticsPage: React.FC = () => {
  ...
  const exportToExcel = () => {
    ...
    try {
      ...
      const tableData = studentsInNotebook.map((student, index) => {
        console.log(`[ClassAnalytics] Estudiante ${index}:`, {
          nombre: student.nombre,
          score: student.score,
          puntosRepasoInteligente: student.puntosRepasoInteligente,
          puntosEstudioActivo: student.puntosEstudioActivo,
          puntosEstudioLibre: student.puntosEstudioLibre,
          puntosQuiz: student.puntosQuiz,
          puntosJuegos: student.puntosJuegos,
          porcentajeDominio: student.porcentajeDominio,
          tiempoEstudio: student.tiempoEstudio
        });
        
        return {
          'Estudiante': student.nombre || 'Sin nombre',
          'Score General': Number(student.score) || 0,
          'Repaso Inteligente': Number(student.puntosRepasoInteligente) || 0,
          'Estudio Activo': Number(student.puntosEstudioActivo) || 0,
          'Estudio Libre': Number(student.puntosEstudioLibre) || 0,
          'Quiz': Number(student.puntosQuiz) || 0,
          'Juegos': Number(student.puntosJuegos) || 0,
          '% Dominio': Number(student.porcentajeDominio) || 0,
          'Tiempo (min)': Number(student.tiempoEstudio) || 0
        };
      });
      ...
      // Obtener el nombre de la materia para el nombre de la hoja y archivo
      const materiaName = materias.find(m => m.id === selectedMateria)?.nombre || 'ClassAnalytics';
      const notebookName = selectedNotebook?.nombre || 'Cuaderno';
      const sheetName = `${materiaName}_${notebookName}`.substring(0, 31); // Excel limita a 31 caracteres
      const fileName = `${materiaName.replace(/\s+/g, '_')}_${notebookName.replace(/\s+/g, '_')}_Estudiantes.xlsx`;
